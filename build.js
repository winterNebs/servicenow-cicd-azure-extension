...
function getVersion(task = "") {
    if ("" === task) {
        return new Promise((resolve, reject) => {
            fs.readFile(
                path.join(__dirname, "src/extension/extension.vsixmanifest"),
                (err, body) => {
                    if (err) {
                        reject(err);
                    } else {
                        let match = body
                            .toString()
                            .match(/<Identity [^>]+Version="(\d+\.\d+\.\d+)"/);
                        if (!match) {
                            reject(
                                "Could not get version from extension.vsixmanifest"
                            );
                        } else {
        ...
        return fse
            .readJson(
                path.join(__dirname, "src/extension/Tasks", task, "task.json")
            )
            .then((json) => [
                +json.version.Major,
                +json.version.Minor,
...
function setVersion(version) {
    const stringVersion = version.join("."),
        objVersion = {
            Major: version[0],
            Minor: version[1],
            Patch: version[2]
    ...
    let promises = tasks.map((task) => {
        ...
        return fse.readJson(fName).then((json) => {
    ...
    promises.push(
        new Promise((resolve, reject) => {
            const fName = path.join(
                __dirname,
                "src/extension/extension.vsixmanifest"
            );
...
function updateVersion() {
    return Promise.all(["", ...tasks].map((task) => getVersion(task))).then(
        (versions) => {
...
function build(version) {
    ...
    return fse
        .ensureDir(path.join(__dirname, "tmp"), 0o2775)
        ...
        .then(() =>
            Promise.all(
                tasks.map((task) =>
                    Promise.all([
                        copy(
                            "src/lib/AzureDevopsPipeline.js",
                            `out/Tasks/${task}/AzureDevopsPipeline.js`
                        ),
                        copy(
                            "src/lib/ServiceNowCICDRestAPIService.js",
                            `out/Tasks/${task}/ServiceNowCICDRestAPIService.js`
                        ),
                        copy("src/lib/index.js", `out/Tasks/${task}/index.js`),
                        copy(`src/lib/${task}.js`, `out/Tasks/${task}/task.js`),
                        copy(
                            "prod_deps/node_modules",
                            `out/Tasks/${task}/node_modules`
                        )
                    ])
                )
            )
        ...
        .then(() =>
            fse.move(
                path.join(__dirname, "tmp/servicenow.vsix"),
                path.join(__dirname, `out/servicenow.extension.${version}.vsix`)
            )
        )
        .then(() =>
            console.log(
                `##vso[task.setvariable variable=artifactName;isOutput=true]servicenow.extension.${version}.vsix`
            )
        );
}
...
// TODO: This should be done using a buildtool to bundle the node modules. This is a hacky work around...
    ...
    return fse
        ...
        .then(() => {
            new Promise(() => {
                ...
                execSync(
                    "cd " + outDir + " && npm install --production --prune"
                );
